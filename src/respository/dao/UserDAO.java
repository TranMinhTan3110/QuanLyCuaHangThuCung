...
public class UserDAO implements DaoInterface<User> {
	...
	public ArrayList<User> getAll() {
		...
		String sql = "SELECT U.id, P.name, P.phone, P.address, U.username, U.password, U.roleName " + "FROM [User] U "
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			while (rs.next()) {
				userList.add(new User(rs.getInt("id"), rs.getString("name"), rs.getString("phone"),
	...
	public User selectByID(int id) {
		String sql = "SELECT p.id, p.name, p.phone, p.address, u.username, u.password, u.roleName " + "FROM Person p "
				+ "JOIN [User] u ON p.id = u.id " + "WHERE p.id = ?";
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			if (rs.next()) {
				...
				return new User(rs.getInt("id"), rs.getString("name"), rs.getString("phone"), rs.getString("address"),
						rs.getString("username"), rs.getString("password"), Role.valueOf(rs.getString("roleName")));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
