...
public class BillDAO implements DaoInterface<Bill> {
	...
	public ArrayList<Bill> getAll() {
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			while (rs.next()) {
				Bill bill = new Bill(rs.getInt("billID"), rs.getInt("orderID"), rs.getDouble("amount"),
						rs.getString("billMethod"), rs.getDate("billTime")
	...
	public Bill selectByID(int id) {
		...
		try (Connection conn = DatabaseConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			if (rs.next()) {
				return new Bill(rs.getInt("billID"), rs.getInt("orderID"), rs.getDouble("amount"),
						rs.getString("billMethod"), rs.getDate("billTime"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
