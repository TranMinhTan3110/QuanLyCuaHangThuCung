...
//    }
	...
	private void loadEmployeesFromDB() {
		...
		for (User user : users) {
			userView.addUserToTable(String.valueOf(user.getId()), user.getName(), user.getPhone(), user.getUsername(),
					user.getPassword(), user.getAddress(), RoleUtil.formatRole(user.getRole()));
		}
	}
...
//            String idStr = userView.getIdField();
		...
		if (name == null || name.trim().isEmpty() || name.equals("Enter Name") || phone == null
				|| phone.trim().isEmpty() || phone.equals("Enter Phone") || password == null
				|| password.trim().isEmpty() || password.equals("Enter Password") || address == null
				|| address.trim().isEmpty() || address.equals("Enter Address") || username == null
				|| username.trim().isEmpty() || username.equals("Enter Username")) {
...
//            }
		...
		if (userService.insert(user)) {
			userView.addUserToTable(String.valueOf(user.getId()), name, phone, username, password, address,
					RoleUtil.formatRole(role));
...
//            Role role = RoleUtil.parseRole(roleStr);
			...
			if (userService.update(user)) {
				userView.updateUserInTable(selectedRow, String.valueOf(user.getId()), name, phone, username, password,
						address, RoleUtil.formatRole(role));
