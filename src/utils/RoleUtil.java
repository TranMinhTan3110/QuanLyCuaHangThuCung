...
public class RoleUtil {
	...
	/**
	 * Chuyển đổi một chuỗi role lấy từ database (NVARCHAR) thành enum Role. Ví dụ:
	 * "ADMIN", "admin" hoặc " admin " sẽ được chuyển thành Role.Admin.
	 *
	...
	public static Role parseRole(String roleStr) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Invalid role string: '" + roleStr + "'. Expected one of: " + Arrays.toString(Role.values()));
		}
	}

	/**
	 * Định dạng lại enum Role thành chuỗi viết thường hoàn toàn để lưu vào database
	 * (nếu cần). Ví dụ: Role.Admin -> "admin"
	 *
